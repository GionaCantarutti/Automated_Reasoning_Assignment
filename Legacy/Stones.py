...
model1 = mzn.Model()
model1.add_file("Stones2.mzn")

model2 = mzn.Model()
model2.add_file("Stones3.mzn")
...
def formatSolution2(stoneList, n, placements, coordinates):
    newPlacements = []
    sides = []
    ## 0 means it's not used. 1 -> only first side used. 2 -> only second side used. 3 -> both sides used
    used = [0 for x in range(len(stoneList))]
    
    placements = numpy.transpose(placements)

    for p in placements:
        print(p)
        for i in range(len(stoneList)):
            if (p[0] == stoneList[i][0] & p[1] == stoneList[i][1] & used[i] != 1 & used[i] != 3):
                newPlacements.append(i+1)
                sides.append(1)
                if (used[i] == 2): used[i] = 3
                if (used[i] == 0): used[i] = 1
            elif (p[1] == stoneList[i][0] & p[0] == stoneList[i][1] & used[i] != 2 & used[i] != 3):
                newPlacements.append(i+1)
                sides.append(2)
